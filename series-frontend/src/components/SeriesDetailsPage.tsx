  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (!createMode && id) setSeries(await api.getSeries(id));
      } catch (e) {
  ...
  return (
    <Container maxWidth="md" sx={{ py: 4 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Button variant="outlined" onClick={() => router.push(backUrl)}>{intl.formatMessage({ id: 'series.back' })}</Button>
        {!createMode && mode === 'view' && <IconButton color="primary" onClick={() => setMode('edit')}><EditIcon /></IconButton>}
      </Box>
      <Typography variant="h4" fontWeight={700} sx={{ mb: 3 }}>{createMode ? intl.formatMessage({ id: 'series.add' }) : series?.title}</Typography>
      {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}
      {(createMode || series) && <SeriesForm series={series} studios={studios} mode={mode} onSubmit={submit} onCancel={() => createMode ? router.push(backUrl) : setMode('view')} />}
