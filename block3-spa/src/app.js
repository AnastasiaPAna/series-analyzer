(function () {
  ...
  const translations = {
    uk: {
      ...
      "hero.eyebrow": "Каталог та аналітика",
      "hero.title": "Відкривай, порівнюй і керуй своїм каталогом серіалів.",
      "hero.text": "Шукай назви, фільтруй за статусом, відстежуй студії та залишай відгуки без переходів між сторінками.",
      "hero.catalog": "Відкрити каталог",
      "hero.tools": "Відкрити статистику",
      ...
      "catalog.statusTitle": "Статус релізу",
      "catalog.previous": "Назад",
      "catalog.next": "Далі",
      "status.all": "Усі",
      "status.finished": "Завершені",
      "status.progress": "Тривають",
      "status.planned": "Анонси",
      ...
      "reviews.composerTitle": "Відгуки до обраного серіалу",
      ...
      "series.titleHint": "2-120 символів, без дивних спецсимволів.",
      "series.genrePickerPlaceholder": "Обери або введи жанр",
      "series.genreAdd": "Додати жанр",
      "series.genreHint": "Можна вибрати від 1 до 3 жанрів. Підказки беруться з уже існуючих жанрів.",
      "series.seasonsHint": "Від 1 до 60 сезонів.",
      "series.ratingHint": "Оцінка від 0 до 10, максимум 1 знак після коми.",
      "series.yearHintDefault": "Якщо рік більший за 2026, це вважається анонсованим релізом.",
      "series.studioHint": "Студія має бути обрана зі списку.",
      "series.title": "Назва",
      "series.genre": "Жанр",
      ...
      "message.filtersReset": "Фільтри оновлено.",
      ...
      "series.progressChip": "Триває",
      "series.plannedChip": "Анонсований",
      "series.idChip": "ID {id}",
    ...
    en: {
      ...
      "hero.eyebrow": "Catalog and insights",
      "hero.title": "Discover, compare and manage your series catalog.",
      "hero.text": "Search titles, filter by status, track studios and leave reviews without leaving the page.",
      "hero.catalog": "Open catalog",
      "hero.tools": "Open statistics",
      ...
      "catalog.statusTitle": "Release status",
      ...
      "reviews.composerTitle": "Reviews for the selected series",
      ...
      "series.titleHint": "2-120 characters, no unusual symbols.",
      "series.genrePickerPlaceholder": "Choose or type a genre",
      "series.genreAdd": "Add genre",
      "series.genreHint": "You can choose from 1 to 3 genres. Suggestions are based on existing genres.",
      "series.seasonsHint": "From 1 to 60 seasons.",
      "series.ratingHint": "Rating from 0 to 10, max 1 decimal place.",
      "series.yearHintDefault": "If the year is after 2026, it is treated as a planned release.",
      "series.studioHint": "Select a studio from the list.",
      "series.title": "Title",
      "series.genre": "Genre",
      ...
      "message.filtersReset": "Filters updated.",
      ...
      "series.progressChip": "In progress",
      "series.plannedChip": "Planned",
      "series.idChip": "ID {id}",
  ...
  function bindEvents() {
    ...
    elements.statusFilters.addEventListener("click", function (event) {
  ...
  function renderStatusFilters() {
    ...
    elements.statusFilters.querySelectorAll("[data-status]").forEach(function (button) {
      button.classList.toggle("active", (button.dataset.status || "all") === state.filters.status);
    });
  }
  ...
  function getFilteredSeries() {
    ...
    let items = state.series.filter(function (item) {
      ...
      const matchesStatus = status === "all" || getSeriesLifecycleStatus(item) === status;
      const matchesYear = !year || String(item.year) === year;
      const matchesRating = !state.filters.minRating || Number(item.rating) >= minRating;
      return matchesSearch && matchesStudio && matchesGenre && matchesStatus && matchesYear && matchesRating;
    });
  ...
  function renderSeries() {
    ...
    elements.seriesGrid.innerHTML = visibleItems
      .map(function (item) {
        ...
        const lifecycle = getSeriesLifecycleStatus(item);
        const lifecycleLabel = getSeriesLifecycleLabel(item);
        ...
        return (
          '<article class="series-card' + (canManage ? ' can-manage' : '') + '">' +
            ...
            '<div class="series-profile">' +
              '<div class="series-profile-copy">' +
                '<span class="series-status-pill ' + lifecycle + '">' + escapeHtml(lifecycleLabel) + "</span>" +
                '<span class="chip accent">' + escapeHtml(t("series.idChip", { id: item.id })) + "</span>" +
              "</div>" +
              '<p class="series-profile-note">' + escapeHtml(item.studio.name) + " · " + escapeHtml(item.genre) + "</p>" +
            "</div>" +
  ...
  function formatStatisticsLabel(attribute, rawValue) {
    if (attribute === "finished") {
      ...
      if (String(rawValue).toLowerCase() === "false") {
        return state.language === "uk" ? "Не завершений" : "Not finished";
      }
    }
  ...
  function getSeriesLifecycleStatus(item) {
    if (item.finished) {
      return "finished";
    }
    if (Number(item.year) > CURRENT_YEAR) {
      return "planned";
    }
    return "progress";
  }

  function getSeriesLifecycleLabel(item) {
    const status = getSeriesLifecycleStatus(item);
    if (status === "planned") {
      return t("series.plannedChip");
    }
    if (status === "progress") {
      return t("series.progressChip");
    }
    return t("series.finishedChip");
  }
  ...
  function renderSelectedGenres() {
    ...
    elements.seriesSelectedGenres.innerHTML = state.selectedSeriesGenres.length
      ...
      : '<span class="text-muted small">' + escapeHtml(state.language === "uk" ? "Оберіть від 1 до 3 жанрів." : "Choose from 1 to 3 genres.") + "</span>";
